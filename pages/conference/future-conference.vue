  ...
  <section>
    <v-container v-if="news.length">
      ...
      <v-row v-if="latestNews.length">
        <v-col cols="12" md="6" v-for="(item,index) in latestNews" :key="index">
          <v-card
            link
          ...
          >
            ...
            >
              ...
              <v-card-title><v-icon dark>mdi-clock-outline</v-icon>{{$moment.utc(item.start).format("LLL")}} - {{$moment.utc(item.end).format("LLL")}}</v-card-title>
            </v-img>
          </v-card>
        </v-col>
      </v-row>
...
export default {
  ...
  data(){
    return{
      items:[{text: 'Главная', disabled: false, href: '/',}, {text: 'Предстоящие Конференции', disabled: true, href: '/',},],
    }
  },
  watch:{
    current_page(){
  ...
  methods:{
    async loadPage(){
      let data = await this.$axios.$get("/future-conference?page=" + this.current_page);
  ...
  async asyncData({$axios}){
    let data = await $axios.$get("/future-conference");
    let latestNews =  await $axios.$get("/future-actual-conference");
    let news = [];
    let current_page = 1;
    let last_page = 1;
    if(data["data"].length){
